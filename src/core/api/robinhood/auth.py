...
class RobinhoodSignatureAuth:
    ...
    def __init__(
        ...
        if not self.api_key:
            raise AuthenticationError("API key is required for Robinhood authentication")

        # Determine authentication method and initialize accordingly
        if self.private_key_b64:
            # Private key authentication: More secure, allows request signing
            self._initialize_private_key_auth()
        elif self.public_key_b64:
            # Public key authentication: Simplified, direct key verification
            self._initialize_public_key_auth()
        else:
            raise AuthenticationError(
                "Either private key (ROBINHOOD_PRIVATE_KEY) or public key (ROBINHOOD_PUBLIC_KEY) "
    ...
    def get_auth_info(self) -> Dict:
        ...
        return {
            ...
            "private_key_prefix": self._private_key[:30] + "..." if self._private_key else None,
            "public_key_prefix": self._public_key[:30] + "..." if self._public_key else None,
            "sandbox": self.sandbox,
            "base_url": self.base_url,
            "auth_type": "private_key" if self._private_key else "public_key",
        }